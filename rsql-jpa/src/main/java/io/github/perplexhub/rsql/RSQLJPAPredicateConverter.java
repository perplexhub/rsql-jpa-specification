...
public class RSQLJPAPredicateConverter extends RSQLVisitorBase<Predicate, From> {
	...
	public RSQLJPAPredicateConverter(CriteriaBuilder builder, Map<String, String> propertyPathMapper,
			List<RSQLCustomPredicate<?>> customPredicates, Map<String, JoinType> joinHints,
			Collection<String> procedureWhiteList, Collection<String> procedureBlackList) {
		this(builder, propertyPathMapper, customPredicates, joinHints, procedureWhiteList, procedureBlackList, false,
				null);
	}
	...
	public RSQLJPAPredicateConverter(CriteriaBuilder builder,
		...
		this(builder, propertyPathMapper, customPredicates, joinHints, proceduresWhiteList, proceduresBlackList,
				strictEquality, likeEscapeCharacter, JsonbConfiguration.DEFAULT);
	}
			...
			if (!mappedProperty.equals(property)) {
				...
				if (!hasPropertyName(mappedProperty, classMetadata)) {
					...
					if (firstTry && mayBeJSonPath.isPresent()) {
						// Try with path mapping that matches just the beginning of the expression if
						// json
				...
				if (isAssociationType(mappedProperty, classMetadata) && !property.equals(resolvedPropertyPath)) {
					...
					if (isOneToAssociationType) {
						if (joinHints.containsKey(keyJoin)) {
							log.debug("Create a join between [{}] and [{}] using key [{}] with supplied hints",
									previousClass, classMetadata.getJavaType().getName(), keyJoin);
						...
						if (!isManyToManyAssociationType(mappedProperty, previousClassMetadata)
							...
							final SingularAttribute id = identifiableType
									.getId(identifiableType.getIdType().getJavaType());
							if (identifiableType.hasSingleIdAttribute() && id.isId()
									&& id.getName().equals(lookAheadProperty)) {
								lookAheadPropertyIsId = true;
							}
						}
						if (lookAheadPropertyIsId || lookAheadProperty == null) {
							log.debug("Create property path for type [{}] property [{}]",
									classMetadata.getJavaType().getName(), mappedProperty);
	 ...
	 * If the node points to a jsonb attribute, it will return a
	 * {@link ResolvedExpression.JsonbPathExpression}<br>
	 * If the node points to a regular attribute or a function, it will return a
	 * {@link ResolvedExpression.PathExpression}
	 * 
		...
		if (selector instanceof Selector.SingleColumnSelector singleColumnSelector) {
			...
			if (JsonbSupport.isJsonType(attribute)) {
				...
				if (type.isPrimitive()) {
					...
					type = RSQLJPASupport.getValueTypeMap().get(type); // if you want to treat Enum as String and apply
																		// like search, etc
				}
			...
			var expression = selector
					.getExpression((column, criteriaBuilder) -> findPropertyPath(column, root).getPath());
	...
	private Predicate expressionPredicate(ComparisonNode node, ResolvedExpression.PathExpression resolvedExpression) {
		...
		if (arguments.size() > 1) {
			...
			if (op.equals(IN_CIRCLE) && arguments.size() == 3) {
				return inCirclePredicate(expression, arguments);
			}
		} else {
	...
	/**
	 * Builds an ST_Distance-based spatial predicate for the =incircle= operator.
	 *
	 * <p>
	 * The three arguments must be, in order:
	 * <ol>
	 ...
	 *
	 * <p>
	 * The resulting SQL is equivalent to:
	 * {@code ST_Distance(path, ST_SetSRID(ST_MakePoint(lon, lat), 4326)) <= radius}
	 *
	 * @param expression the JPA path expression pointing to the spatial (Point)
	 *                   column
	 * @param arguments  the three string arguments from the RSQL node
	 * @return a JPA {@link Predicate} that checks whether the point is within the
	 *         radius
	 * @throws IllegalArgumentException if the coordinate or radius values cannot be
	 *                                  parsed
	 */
	private Predicate inCirclePredicate(Expression<?> expression, List<String> arguments) {
		double lat, lon, radius;
		try {
			lat = Double.parseDouble(arguments.get(0).trim());
			lon = Double.parseDouble(arguments.get(1).trim());
			radius = Double.parseDouble(arguments.get(2).trim());
		} catch (NumberFormatException e) {
			throw new IllegalArgumentException(
					"=incircle= operator requires numeric values for lat, lon, and radiusMeters", e);
		}
		...
		Expression<Double> distanceExpr = builder.function(
				"ST_Distance",
				Double.class,
				expression,
				builder.literal(center));

		return builder.lessThanOrEqualTo(distanceExpr, radius);
	}
