...
public class RSQLJpaPredicateConverter extends RSQLCommonVisitorBase<Predicate, Root> {
	...
	public Predicate visit(ComparisonNode node, Root root) {
		...
		} else if (RSQLSpecificationSupport.getValueTypeMap().containsKey(type)) {
			type = RSQLSpecificationSupport.getValueTypeMap().get(type); // if you want to treat Enum as String and apply like search, etc
		}
